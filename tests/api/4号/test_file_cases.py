...
def _make_forbidden_file() -> Path:
    """生成不支持类型的临时文件"""
    if "forbidden" not in _generated_files:
        f = tempfile.NamedTemporaryFile(suffix=".exe", delete=False)
        f.write(os.urandom(1024))
        f.close()
        _generated_files["forbidden"] = Path(f.name)
    return _generated_files["forbidden"]
    ...
    if params:
        for key, val in params.items():
            if isinstance(val, str) and val.startswith("{") and val.endswith("}"):
                placeholder = val[1:-1]
                if placeholder in uploaded_info:
...
def uploaded_file_info(auth_client: ApiClient) -> dict:
    ...
    if items:
        ...
        return {
            "uploaded_file_id": first.get("id", 0),
            "uploaded_file_ids": str(first.get("id", 0)),
            "uploaded_filename": first.get("name", ""),
        }
    return {"uploaded_file_id": 0, "uploaded_file_ids": "0", "uploaded_filename": "test.png"}
...
def test_upload_and_download_integrity(auth_client: ApiClient):
    ...
    with allure.step("清理上传的文件"):
        auth_client.request("DELETE", "/cms/file", params={"file_ids": str(file_id)})
...
def test_upload_dedup(auth_client: ApiClient):
    ...
    # 清理
    auth_client.request("DELETE", "/cms/file", params={"file_ids": str(file_id1)})
...
def test_upload_to_folder_and_move(auth_client: ApiClient):
    ...
    items = body_list.get("data", {}).get("items", []) if isinstance(body_list.json().get("data"), dict) else []
    if not items:
        items = body_list.get("items", [])
    folder_id = None
    for item in items:
        if item.get("name") == "4号测试文件夹" and item.get("extension") is None:
