...
def _concurrent_request(client: ApiClient, method: str, path: str, json_body=None, params=None) -> tuple:
    """单次并发请求"""
    start = time.perf_counter()
    try:
...
@pytest.mark.perf
def test_concurrent_login(client: ApiClient):
    """10 并发登录测试"""
...
@pytest.mark.perf
def test_concurrent_query(auth_client: ApiClient):
    """20 并发查询测试"""
    ...
    with allure.step("20 并发查询文件类型"):
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(_concurrent_request, auth_client, "GET", "/cms/file/types")
                for _ in range(20)
            ]
            for f in concurrent.futures.as_completed(futures):
                results.append(f.result())
