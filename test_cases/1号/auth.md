...
> 责任人：1号  
> 测试基线：`/apispec_1.json`  
> 覆盖范围：令牌生成、令牌校验、用户名登录、邮箱登录、手机号登录、异常登录、受限接口访问
...
| TC-AUTH-017 | POST | `/v1/token`        | 参数校验     | 空账号登录校验            | 已执行 `fake.py` | `account=""` `secret=123456` `type=100`           | 返回账户不为空或参数校验失败     | P0  |
| TC-AUTH-018 | POST | `/v1/token`        | 参数校验     | 空密码登录校验            | 已执行 `fake.py` | `account=user` `secret=""` `type=100`             | 返回密码错误，不生成有效 Token | P0  |
| TC-AUTH-019 | POST | `/v1/token`        | 参数校验     | 缺少登录类型             | 已执行 `fake.py` | `account=user` `secret=123456` 缺少 `type`          | 返回参数校验失败           | P0  |
| TC-AUTH-020 | POST | `/v1/token/verify` | Token 校验 | 未携带 Token 访问校验接口   | 无             | `token` 为空                                        | 返回校验失败             | P0  |
| TC-AUTH-021 | POST | `/v1/token/verify` | Token 校验 | 伪造 Token 访问校验接口    | 无             | `token=fake-token`                                | 返回token失效          | P0  |
| TC-AUTH-022 | POST | `/v1/token/verify` | Token 校验 | 有效 Token 访问校验接口    | 已登录并获取 Token  | `token=<valid-token>`                             | 接口正常返回解析结果         | P0  |
| TC-AUTH-023 | GET  | `/v1/user`         | 登录态访问    | 有效 Token 查询当前用户成功  | 已登录并获取 Token  | Header：`Authorization` 携带有效 Token                 | 返回当前用户信息           | P0  |
| TC-AUTH-024 | GET  | `/v1/user`         | 登录态访问    | 未携带 Token 查询当前用户失败 | 无             | 不携带 Token                                         | 返回token无效          | P0  |
| TC-AUTH-025 | GET  | `/v1/user/auths`   | 权限访问     | 普通管理员查询自身权限成功      | 已用 `admin` 登录 | Header：`Authorization` 携带 admin Token             | 返回权限列表             | P1  |
| TC-AUTH-026 | GET  | `/v1/user/auths`   | 权限访问     | 未分配权限组的普通用户查询权限返回空列表 | 已用 `user` 登录  | Header：`Authorization` 携带 user Token              | 返回 `{"items": []}` | P1  |


## 回归建议

- 核心冒烟：`TC-AUTH-001`、`TC-AUTH-004`、`TC-AUTH-008`、`TC-AUTH-022`、`TC-AUTH-023`
- 登录方式覆盖：`TC-AUTH-001` ~ `TC-AUTH-010`
- 登录失败类用例：`TC-AUTH-011` ~ `TC-AUTH-019`
- Token 校验：`TC-AUTH-020` ~ `TC-AUTH-022`
- 受限接口访问：`TC-AUTH-023` ~ `TC-AUTH-026`
