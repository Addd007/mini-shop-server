...
| TC-USER-019 | PUT    | `/v1/user/unbind`   | 账号解绑 | 解绑不存在的账号             | 已登录                 | `type=102`                                                                                                          | 解绑失败返回未查询到数据                 | P1  |
| TC-USER-020 | PUT    | `/v1/user/unbind`   | 账号解绑 | 解绑已绑定的邮箱             | 已登录                 | `type=101`                                                                                                          | 解绑成功                         | P1  |
| TC-USER-021 | PUT    | `/v1/user/unbind`   | 账号解绑 | 解绑已绑定的手机号            | 已登录                 | `type=102`                                                                                                          | 解绑成功                         | P1  |
| TC-USER-022 | PUT    | `/v1/user/bind`     | 账号绑定 | 绑定未占用邮箱成功            | 已登录                 | `account=new_bind_email@qq.com` `type=101`                                                                          | 绑定成功                         | P1  |
| TC-USER-023 | PUT    | `/v1/user/bind`     | 账号绑定 | 绑定未占用手机号成功           | 已登录                 | `account=19900000009` `type=102`                                                                                    | 绑定成功                         | P1  |
| TC-USER-024 | PUT    | `/v1/user/bind`     | 账号绑定 | 已绑定邮箱时再次绑定邮箱失败       | 已用 `user` 登录且已绑定邮箱  | `account=another_email@qq.com` `type=101`                                                                           | 返回当前用户已绑定邮箱，不新增多条邮箱身份        | P1  |
| TC-USER-025 | PUT    | `/v1/user/bind`     | 账号绑定 | 已绑定手机号时再次绑定手机号失败     | 已用 `user` 登录且已绑定手机号 | `account=19900000009` `type=102`                                                                                    | 返回当前用户已绑定手机号，不新增多条手机号身份      | P1  |
| TC-USER-026 | PUT    | `/v1/user/bind`     | 账号绑定 | 绑定其他用户已占用邮箱失败        | 已用 `user` 登录        | `account=777@qq.com` `type=101`                                                                                     | 返回邮箱已被使用，不返回服务器端异常           | P1  |
| TC-USER-027 | PUT    | `/v1/user/bind`     | 账号绑定 | 绑定其他用户已占用手机号失败       | 已用 `user` 登录        | `account=19900000002` `type=102`                                                                                    | 返回手机号已被使用，不返回服务器端异常          | P1  |
| TC-USER-028 | DELETE | `/v1/user`          | 注销账号 | 注销当前账号               | 已登录                 | 无                                                                                                                   | 注销成功或返回确认结果                  | P2  |
| TC-USER-029 | POST   | `/v1/user`          | 注册恢复 | 用户名+手机号匹配已删除账号时复用    | 存在已删除账号且用户名、手机号均同名  | `nickname=普通用户` `username=user` `mobile=19900000003` `email=new@qq.com` `password=123456` `confirm_password=123456` | 复用已删除账号，补充邮箱/更新信息            | P1  |
| TC-USER-030 | POST   | `/v1/user`          | 注册恢复 | 用户名+邮箱匹配已删除账号时复用     | 存在已删除账号且用户名、邮箱均同名   | `nickname=普通用户` `username=user` `email=111@qq.com` `mobile=10000000000` `password=123456` `confirm_password=123456` | 复用已删除账号，补充手机号/更新信息           | P1  |
